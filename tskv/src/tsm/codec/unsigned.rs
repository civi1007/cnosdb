...
pub fn u64_zigzag_simple8b_decode(
    ...
    if src.is_empty() {
        return Ok(Arc::new(UInt64Array::from(vec![] as Vec<u64>)));
    }
    let array_ref = super::integer::i64_zigzag_simple8b_decode(src, bit_set)?;
    Ok(array_ref)
}
...
pub fn u64_pco_decode(
    ...
    Ok(super::integer::i64_pco_decode(src, bit_set)?)
}
...
pub fn u64_without_compress_decode(
    ...
    Ok(super::integer::i64_without_compress_decode(src, bit_set)?)
}
